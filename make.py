...
for app in CFE_APPS:
    try:
        if not os.path.exists(COMPOSITE_IMPL_NO_INTERFACE_DIR + app):
            print "Failed to copy '{}', error: no interface directory doesn't exist.".format(app)
            continue
        ...
        if not os.path.exists(CFE_MAKE_ROOT + app):
            print "Failed to copy '{}', error: make directory doesn't exist.".format(app)
            continue
    ...
    except (IOError, sp.CalledProcessError) as e:
            print "Failed to copy '{}', error: '{}'".format(app, e)
            continue
