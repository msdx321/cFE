...
retryget:
   ...
   if (Status != OS_SUCCESS) {
      if (Status == OS_ERR_NAME_NOT_FOUND) {
         ...
         Status = OS_QueueCreate(&sensordump_qid, I42_SENSOREMU_Q_NAME, I42_SENSOREMU_Q_DEPTH, I42_SENSOREMU_DATASZ, 0);
         if (Status != OS_SUCCESS) OS_printf("Failed to create the sensor dump queue[%d]!\n", Status);
         if (Status == OS_ERR_NAME_TAKEN) goto retryget;
      }
